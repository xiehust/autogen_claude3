...
def setup_chatgroups(st_session,llm_config):
    ...
    name="FinanceAdvisor",
    llm_config=llm_config,
    description="""FinanceAdvisor. You follow an approved plan. You are able to categorize papers after seeing their abstracts printed. You don't write code.""",
    )

    finance.register_reply(
        [autogen.Agent, None],
        reply_func=print_messages, 
        config={"session_messages": st_session,"avatar":"🐶"},#https://emojicopy.com/
    )
    ...
    planner = ConversableAgent(
        name="Planner",
        code_execution_config=False,
        description='''Planner. Suggest a plan. Revise the plan based on feedback from admin, until admin approval.
The plan may involve an engineer who can write code and a scientist who doesn't write code.
    ...
    ## 控制流转：例如 engineer节点只能在planner和executor后面
    allowed_transitions = {
        user_proxy:[user_proxy,planner],
        planner: [user_proxy,executor],
        engineer: [executor, planner],
        executor: [engineer],
    }
    
    
    groupchat = GroupChat(agents=[user_proxy, planner, engineer,executor ],
